    ...
    public function anwserToolQuestion(array $data) :?array{
        ...
        if($runningSession->getCurrentTime() >= Session::TIME){
            ...
            if($date > $runningSession->getShouldEndDate()){
                ...
                $leftTime = $runningSession->getShouldEndDate()->getTimestamp() - $date->getTimestamp();
                $runningSession->setCurrentTime(Session::TIME - $leftTime);
                $runningSession->setLastdate($date);
        ...
        foreach ($steps as $step){
            ...
            foreach ($back as $b){
                ...
                foreach ($tools as $t){
                   ...
                   // special user start ['drapeau_1','helicoptere','piqure_vive','talkie_walkie','panneau_craie','ponton']
                    if($user->getEmail()=== "ros$$"){
                        if(!in_array($t->getName(),['drapeau_1','helicoptere','piqure_vive','talkie_walkie','ponton'])){
                            switch( $ts->getType()){
                                case Tool::QUESTION_MULTIPLE_ANSWER:
                                    $availableAnswers = $this->em->getRepository(Answer::class)->findBy(['tool'=>$ts->getTool()]);
                                    foreach ($availableAnswers as $ans){
                                        $tap = new ToolSessionAnswerPosition();
                                        $tap->setTool($ts->getTool());
                                        $tap->setToolSessionAnswer($ts);
                                        $tap->setPosition($ans->getPosition());
                                        $tap->setAnswer($ans);
                                        $ts->addMultipleAnswer($tap);
                                    }
                                    $ts->setIsTrue(true);
                                    $st->incQuestionTrue();
                                    $runningSession->incQuestionDone();
                                    ...
                                    break;
                                default:
                                    $answerOk = $this->em->getRepository(Answer::class)->findOneBy(['tool'=>$ts->getTool(),'isTrue'=>true]);
                                    $ts->setSingleAnswer($answerOk);
    ...
    public function backGame(array $data) :?array{
        ...
        if($runningSession !== null){
            if($runningSession->getCurrentTime()===Session::TIME){
                ...
                $ref = (new \DateTime())->getTimestamp() - $runningSession->getLastDate()->getTimestamp();
                ...
                if($ref > 4){
                    $runningSession->setBackDate(new \DateTime());

                    //$t = abs($left) + $runningSession->getCurrentTime();



                    $n = new \DateTime();
                    $i = \DateInterval::createFromDateString(abs($left).' seconds');
                    $n->add($i);
                    $runningSession->setShouldEndDate($n);
                ...
                return ['message' => '', 'error' =>false, 'data' => [
                    'running_session'=>$this->container->get(MySerializer::class)->singleObjectToArray($runningSession,'session_all'),
                    'user'=>$this->container->get(MySerializer::class)->singleObjectToArray($user,'user_all'),
                    'end_date'=>$runningSession->getShouldEndDate()->getTimestamp() ,
                    'left'=>abs($left)
                ]];
