...
import initFoundationModules from '@virgilsecurity/core-foundation/node.asmjs.es.js';
import { ModuleInitializer } from '@virgilsecurity/init-utils';
import { NodeBuffer, toBuffer, dataToUint8Array } from '@virgilsecurity/data-utils';
...
const isRSAKeyPairType = (keyPairType) => keyPairType === KeyPairType.RSA_2048 ||
    keyPairType === KeyPairType.RSA_3072 ||
    keyPairType === KeyPairType.RSA_4096 ||
    keyPairType === KeyPairType.RSA_8192;
const isCompoundKeyPairType = (keyPairType) => keyPairType === KeyPairType.CURVE25519_ROUND5_ED25519_FALCON ||
...
})(VirgilCryptoErrorStatus || (VirgilCryptoErrorStatus = {}));
...
class VirgilStreamCipher {
    ...
    ensureLegalState() {
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot use cipher after the 'dispose' method has been called.");
        }
        if (this._isFinished) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot use cipher after the 'final' method has been called.");
        }
    }
    ensureIsRunning() {
        if (!this._isRunning) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot use cipher before the 'start' method.");
        }
    }
}
...
class VirgilStreamDecipher {
    ...
    getSignature() {
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot get signature after the 'dispose' method has been called.");
        }
        if (!this._isFinished) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot get signature before the 'final' method has been called.");
        }
...
class VirgilStreamDecryptAndVerify {
    ...
    verify(arg0, arg1 = true) {
        ...
        validatePublicKeysArray(publicKeys);
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot verify signature after the 'dispose' method has been called.");
        }
        if (!this._isFinished) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot verify signature before the 'final' method has been called.");
        }
        let signerInfoList;
        try {
...
class VirgilStreamSigner {
    ...
    update(data) {
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. Cannot use signer after the 'dispose' method has been called.");
        }
        const myData = dataToUint8Array(data, 'utf8');
    ...
    sign(privateKey, final = true) {
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. The VirgilStreamSigner has been disposed. Pass 'false' as the second argument to the 'sign' method if you need to generate more than one signature.");
        }
        validatePrivateKey(privateKey);
...
class VirgilStreamVerifier {
    ...
    verify(publicKey, final = true) {
        if (this._isDisposed) {
            throw new VirgilCryptoError(VirgilCryptoErrorStatus.STREAM_ILLEGAL_STATE, "Illegal state. The VirgilStreamVerifier has been disposed. Pass 'false' as the second argument to the 'verify' method if you need to verify with more than one public key.");
        }
...
export { HashAlgorithm, KeyPairType, VirgilCrypto, VirgilCryptoError, VirgilCryptoErrorStatus, VirgilPrivateKey, VirgilPublicKey, VirgilStreamCipher, VirgilStreamDecipher, VirgilStreamSigner, VirgilStreamVerifier, getFoundationModules, hasFoundationModules, initCrypto, moduleInitializer, setFoundationModules };
